...
func NewCancelReader(file File) (cancelReader, error) {
	...
	if f, ok := file.(*os.File); !ok || f.Fd() != os.Stdin.Fd() {
		return newFallbackCancelReader(file)
	}

	// if we cannot get the console mode for stdin, it is not a console
	if m := uint32(0); syscall.GetConsoleMode(syscall.Handle(file.Fd()), &m) != nil {
		return newFallbackCancelReader(file)
	}
