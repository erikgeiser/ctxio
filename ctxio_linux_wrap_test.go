...
func TestWrapFilePipe(t *testing.T) {
	...
	if _, ok := cio.(*epollIO); !ok {
		t.Errorf("WrapFile(pipe) = %T, want *epollIO", cio)
	}
}
...
func TestWrapFileRegular(t *testing.T) {
	...
	if _, ok := cio.(*selectIO); !ok {
		t.Errorf("WrapFile(regular file) = %T, want *selectIO", cio)
	}
}
...
func TestWrapFileTTY(t *testing.T) {
	...
	if _, ok := cio.(*epollIO); !ok {
		t.Errorf("WrapFile(/dev/tty) = %T, want *epollIO", cio)
	}
}
...
func TestWrapConnTCP(t *testing.T) {
	...
	if _, ok := cio.(*epollIO); !ok {
		t.Errorf("WrapConn(tcp) = %T, want *epollIO", cio)
	}
}
